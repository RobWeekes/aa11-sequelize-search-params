...
app.get('/musicians', async (req, res, next) => {
    ...
    // using ternary, if page is undef, default to page = 1 / otherwise page = page requested
    const size = req.query.size === undefined ? 5 : parseInt(req.query.size); // default limit 5
    if (page >= 1 && size >= 1) {
        query.limit = size;  // limit the data shown to the size requested
    ...
    // if req query exists, add to the "where" query key
    if (firstName) { query.where = { firstName: req.query.firstName } }
    ...
    if (lastName) { query.where = { lastName: req.query.lastName } }
    ...
    // if req query exists, push Band model into query's include array
    if (bandName) query.include.push({ model: Band, where: { name: req.query.bandName } })
    ...
    */
    ...
    // Your code here
    // const instruTypes = [];    // no need to create a new array!
    // matching instru types are being filtered & pushed into the response
    // on lines 90-93 and 148 \/
    if (req.query.instrumentTypes) {   // only execute if instrumentTypes are passed in req.query
        // req.query.instrumentTypes.forEach(type => instruTypes.push(type))
        console.log('intrumentType array: ', req.query.instrumentTypes);
    ...
    if (req.query.musicianFields) {
        ...
        console.log(musicianFields);    // log the query params

        if (musicianFields.includes('all')) {
            // if (musicianFields == 'all') { // also works but 'all' must be the only musician field
            // attributes: { } // Don't limit attributes: will return all
        }

        else if (musicianFields.includes('none')) {
            query.attributes = ['id']; // Return no attributes
        }

        else {
            query.attributes = musicianFields; // Only return specified fields in req query
        }
    }
